//"use strict";
//This function is another method to collect data from youtube.
...
function insertVideo() {
    ...
    (async function loop() {
        ...
        for (let i = 0; i < 100; i++) {
            ...
            await fetchVideoInfo(addressArray[i]).then(function (videoInfo) {
                if (String(videoInfo.thumbnailUrl).includes("maxresdefault")) {
    ...
    /*
    //callback cause block, so async should be used
    (async function loop() {
        for (let i = 0; i < addressArray.length; i++) {
            ...
            fetchVideoInfo(addressArray[i]).then(function (videoInfo) {
                ...
                //           console.log(String(videoInfo.thumbnailUrl).includes("maxresdefault"));
                if (String(videoInfo.thumbnailUrl).includes("maxresdefault")) {
                    ps.run(addressArray[i], videoInfo.title.replace("'", "\'").replace("&", "&amp;"),
                        videoInfo.owner.replace("'", "\'").replace("&", "&amp;"), videoInfo.views,
                        videoInfo.datePublished, indexArray[i]);
